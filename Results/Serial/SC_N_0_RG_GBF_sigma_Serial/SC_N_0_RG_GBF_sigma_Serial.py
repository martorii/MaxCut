...
Original file is located at
    https://colab.research.google.com/drive/1PN-Bg3WFQTNuR7IISE0MjDOtKzwcj8oO
"""
...
# Max-cut functions
def InitializeGraph(n, min_weight, max_weight, dropout, seed = -1, graph_type="complete"):
    # Randomness
...
def InitializeFlatGraph(n, min_weight, max_weight, dropout = 0, seed = -1, graph_type = "complete"):
    if graph_type == "complete":
        if seed == -1:
            W = np.random.uniform(low=min_weight, high=max_weight, size=n)
        else:
...
# Plot results
def PlotGraph(W, partition = None):
...
nodes, smoothed_complexities = RandomGridSmoothedComplexitySigma(min_nodes, max_nodes, num_nodes, initial_partition_type, k, dropout, 
                                  min_weight, max_weight, n_iterations, iters_for_w, heuristic, 
                                  sigmas, plotMaxEvolution = False, plotWorstGraph = False, storeCSV=True)
